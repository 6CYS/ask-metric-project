...
<template>
  <div class="mt-3 space-y-2 text-sm" aria-label="工具计算结果">
    <p v-if="calculation.status !== 'succeeded'" class="text-muted-foreground">计算未完成：{{ calculation.message || '请检查计算条件后重试。' }}</p>
    <template v-else>
      <p v-for="result in calculation.results" :key="result.name">
        <span class="text-muted-foreground">{{ result.label }}：</span>
        <strong class="font-medium tabular-nums">{{ result.display_value }}{{ result.unit }}</strong>
      </p>
      <details class="text-muted-foreground">
        <summary class="cursor-pointer focus-visible:outline-2 focus-visible:outline-ring">查看计算口径和数据来源</summary>
