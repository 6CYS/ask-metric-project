...
<template>
  <section class="space-y-1 text-sm leading-7">
    ...
    <p class="text-muted-foreground"><template v-if="hasInlineChoices">可点击上方候选名称确认，也可在下方输入框补充。</template><template v-else>请在下方输入框直接补充。</template><template v-if="catalogKinds.length">输入<template v-for="(kind, index) in catalogKinds" :key="kind">{{ index ? '、' : '' }}“/{{ kind }}”</template>可搜索并添加多项，一起发送。</template></p>
