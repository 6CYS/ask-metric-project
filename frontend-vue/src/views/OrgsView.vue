...
const emptyDescription = computed(() => hasSearch.value ? "请调整关键词，或打开「显示已停用」后再试。" : showDisabled.value ? "新增第一条机构后，即可开始沉淀机构标准名和别名。" : "打开「显示已停用」开关可查看已停用项，或直接新增一个机构。")
...
<template>
  <AppShell>
    <PageHeader eyebrow="Organization Catalog" title="机构别名" description="维护机构编号、标准机构名和常用别名，为后续机构归一和问数消歧提供统一目录。" :stats="stats">
      <template #actions><BaseButton @click="openCreateDialog"><Plus />新增机构</BaseButton><BaseButton variant="outline" :disabled="resource.isRefreshing.value" @click="resource.load(true)"><RefreshCw :class="resource.isRefreshing.value && 'animate-spin'" />刷新</BaseButton></template>
    </PageHeader>
    ...
    >
      ...
      <div class="overflow-hidden rounded-lg border">
        <div class="overflow-x-auto"><table class="w-full min-w-[720px] table-fixed text-sm">
          <thead><tr class="border-b bg-muted/45"><th class="w-36 table-head">机构编号</th><th class="w-48 table-head">标准机构名</th><th class="w-64 table-head">别名</th><th class="w-24 table-head">状态</th><th class="w-40 table-head">更新时间</th><th class="w-32 table-head text-right">操作</th></tr></thead>
          <tbody><tr v-for="org in pageItems" :key="org.org_code" class="border-b last:border-0 hover:bg-muted/35">
            ...
            <td class="table-cell"><div class="flex justify-end gap-1"><BaseButton variant="ghost" size="icon" :disabled="resource.isMutating.value" aria-label="编辑机构" @click="openEditDialog(org)"><Pencil /></BaseButton><BaseButton variant="ghost" size="icon" :disabled="resource.isMutating.value" :aria-label="org.enabled ? '停用机构' : '启用机构'" @click="toggleEnabled(org)"><LoaderCircle v-if="togglingOrgCode === org.org_code" class="animate-spin" /><PowerOff v-else-if="org.enabled" /><Power v-else /></BaseButton><BaseButton variant="ghost" size="icon" :disabled="resource.isMutating.value" aria-label="删除机构" @click="deletingOrg = org"><Trash2 /></BaseButton></div></td>
          </tr></tbody>
    ...
    <BaseModal v-model:open="isDialogOpen" :busy="resource.isMutating.value" size="lg" :title="editingOrg ? '编辑机构' : '新增机构'" :description="editingOrg ? '修改机构标准名和别名。别名用中文逗号或英文逗号分隔。' : '新增机构标准名和常用别名，后续问数解析会复用这套机构目录。'">
    ...
    <ConfirmDialog :open="Boolean(deletingOrg)" title="删除机构" :busy="resource.isMutating.value" @update:open="!$event && (deletingOrg = null)" @confirm="confirmDelete">即将软删除机构 <strong class="font-medium text-foreground">「{{ deletingOrg?.org_name }}」</strong>。删除后该机构会被标记为停用，历史配置数据会保留。</ConfirmDialog>
  </AppShell>
</template>
