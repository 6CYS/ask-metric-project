...
onMounted(async () => {
  try {
    ...
    passwordLoginEnabled.value = config.password_login_enabled ?? !config.sso_enabled
  ...
  <main class="flex min-h-screen items-center justify-center bg-background p-8">
    <section class="max-w-lg space-y-5 text-center">
      ...
      <div class="flex justify-center gap-5">
        <RouterLink v-if="passwordLoginEnabled" to="/login" class="underline">测试账号登录</RouterLink>
