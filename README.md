...
权限、查询与权威结果。支持单次可信问数、当前任务澄清的自然补充，以及引用一笔已完成查询组合修改机构、指标、日期和查询目标的追问，
支持查询有数据的月份、日期及最早/最新期间；归因请求仍明确返回当前不支持。agent 通过受治理接口复用可信查询、结果证据、权限和
...
5. 每次新问题独立查询；当前任务缺少条件时继续澄清。智能助手可显式引用已完成任务，组合修改机构、指标、日期和查询目标；历史结果通过只读工具回放。旧的隐式跨任务接口已退出，当前协议见 [Agent 服务说明](agent-service/README.md)。
