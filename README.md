...
初次维护代码可从 [关键代码阅读指南](docs/关键代码阅读指南.md) 开始，按启动、提问、多轮、查询执行和历史结果的顺序阅读；其中附有本项目 `lambda`、依赖注入、事务和向量缓存的写法说明。

当前运行版本维护可信问数、多轮条件修改和历史结果操作。旧归因分析原型已退出运行链，
...
5. 需要多轮问答时显式配置 `MULTITURN_V2_ENABLED=true`，默认模板不会启用全部功能。
...
- [本地部署指南](docs/本地部署指南.md)：首次安装、密钥、目录、账号、模型和多轮开关；
