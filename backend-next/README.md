...
只更换指标时继承当前查询已确认的日期、机构和操作，即使上一轮返回零行数据。
完整新问题仍独立理解；指标有歧义时只澄清指标，不要求重复已确认条件。

多轮模型输入复用单轮查询的指标实体占位保护，模型返回后恢复原文并进行目录校验。
新增操作或比较参数必须引用实体名称以外的用户指令；条件来源与新查询分类冲突时，
走现有契约纠错重试，持续冲突则停止执行，不默默删除操作或猜测比较日期。
该契约由 `multiturn_context_patch` 提示词提供，升级时须同步实际 `PROMPT_CONFIG_PATH`
指向的持久化配置，不能只更新仓库默认配置。仓库默认提示词只保留查询和历史结果协议，以及对当前不支持目标的识别。

## 旧归因原型退出与升级

当前版本仅维护可信问数、多轮条件修改和历史结果操作。原 LangGraph 归因运行器、
...
2. 合并仓库 `config/prompts.json` 到实际 `PROMPT_CONFIG_PATH` 指向的文件：更新
   `multiturn_context_patch` 为 3.2.0 的查询协议，删除 `analysis_target` 和 `analysis_action`。
   保留已经维护的目录保护、澄清文案和其他自定义配置，不整份覆盖生产持久配置。
   新协议删除了分析历史占位符；管理页面不允许变更占位符集合，须通过受控的配置文件发布完成。
   `config/prompts.query-only.json` 保留为已有部署的兼容路径，与默认模板同步为纯问数协议；
   已使用该路径的环境无需改路径，仍须合并实际持久配置。多轮条件修改继续启用 `MULTITURN_V2_ENABLED=true`。
3. 重新安装当前依赖并同步部署前后端，重启后验证查询、条件追问、澄清、历史结果读取与导出。
...
旧分析记录不会成为普通多轮查询的锚点，前端不再提供其澄清交互。
