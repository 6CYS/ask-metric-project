...
新查询的调试信息包含代入参数后的 `rendered_sql`，供排查使用；实际执行仍使用原模板和
绑定参数，展示文本不参与执行。历史只保存模板名的记录不会自动补出SQL。
取值、趋势及两期比较按请求的机构、指标和显式日期/月份窗口提示缺失项；零值算有效数据。
连续区间只核对区间覆盖，不假定每天都应有记录；排名结果未满TopN或TopN覆盖全部
指定机构时，按指标说明缺席机构。已满TopN时不把未入选机构当成缺数据；结果截断也不误报。
多于5家机构的缺失说明按日期与指标归并；全范围缺失只说明机构数量，部分缺失保留机构名单。
当少数机构有数据、其余多数机构无数据时，用“其余N家机构在该日期未查询到该指标数据”
归并说明。趋势按机构与指标核对实际统计日，连续无记录日期归并为区间；仅一个统计日时
明确无法判断变化趋势，不将无记录解释为零或报送异常。截断结果不据此推断完整日期覆盖。
...
槽位提示词 `1.4.2` 使用本次请求的临时实体编号 `M1/O1`；正式编码、名称及机构候选
不再发送给槽位模型，避免编码中的 `RNKN` 等缩写重新泄漏操作含义。未覆盖的实体文字
仍由模型提取，后端目录负责消歧；已确认的多个指标不会重新拼接成一个待匹配短语。
`ops` 只能表达实体之外的请求语义（例如“谁最高”“逐月变化”），不能从指标或机构
推断操作。非空 `ops` 必须在 `options.operation_evidence` 中按顺序提供对应的连续原文，
后端检查原文位于实体之外；依据缺失、指标短语含实体标签或包含复合操作时最多复核一次，
真实复合请求仍保留给能力校验，操作依据仍不合规则澄清 `ops`，
不会清空操作再执行取值。重复抽取仅传本次问题及校验反馈，不引入历史问数条件。
原文出处校验不等于语义准确率保证，仍需真实模型回归。
`1.4.1` 合并重复约束，保留整月、独立月份窗口及比较依据示例；指定区间内最新
仍保留该区间，只有未限定日期的最新请求使用 `latest`。列出多个日期的值不自动构成趋势。

意图路由提示词 `1.3.0` 只要求 `{"intent":"metric_query"}`，不再要求模型自报
`confidence`；该数值没有参与路由或查询判断。后端兼容旧模板返回的额外字段，
新诊断记录只保存校验后的意图，不记录自报分数；历史诊断不改写。
部署时同步实际 `PROMPT_CONFIG_PATH` 的 `intent_routing.system` 和版本号，
持久配置未更新时虽可兼容解析，但旧模型请求仍会要求输出该分数。
多个显式日期或窗口由模型选择，后端校验其合法性并统一 `time` 包络，避免冗余字段矛盾；
有原文依据的全目录范围不会因为同时返回 `missing=orgs` 再要求用户逐个列举机构。

升级须同时合并实际 `PROMPT_CONFIG_PATH` 中 `slot_extraction.system`、`user_template`
及版本号；两份仓库模板的槽位契约相同，部署仍保留管理员其他自定义配置。旧持久提示词
不会自动覆盖，不更新可能导致带操作的请求要求澄清。历史已保存的结果不重算。
...
此契约通过现有 `slot_frame_schema_json` 注入槽位提示词，不改变模型 HTTP 接口或认证方式，
...
普通取值和历史时序部分返回时，后端按请求的机构、指标组合提示当前时间范围内缺失项；
不补零、不将其他机构的值代替缺失项。排名未入选和结果截断不使用该缺项判断。

新执行的查询在 API、任务结果和下载明细中保留换算、展示舍入前的数值；数据库 Decimal
以字符串序列化，保留原精度。前端表格按原单位显示：一般指标保留两位小数，户数和排名等整数指标显示整数，不展示单位列；显示格式不回写原始值。
历史期间对比仍展示本期值、基期值和派生差值/变化率，其中原值不换算，变化率保持计算所得比例。
历史机构对比的下载明细与页面一样使用逐条结果，不以派生比较记录替代原始记录。
...
新查询统一拒绝 `period_compare`、`entity_compare` 及两期比较查询形态，不进入业务SQL执行；已有正式增幅、排名指标仍可按普通取值查询。历史比较结果保留查看和导出。
槽位提示词1.4.2明确相对年份也是完整日期条件，按业务当前日期解析为对应自然年区间；不要求额外补月份。部署时合并实际持久提示词，不以本地模板更新代替现场配置更新。

### 原文明示日期合法性校验

实体保护后的原文中，明确年月日、月日及月份独立接受日历合法性校验；该检查不依赖模型是否返回空日期。非法日期即使被模型改为合法值也须澄清，保留已确认机构与指标，并清除模型派生的时间选取项。数字按完整片段读取，支持常用中文月份/日数及全角数字；多日期逐项检查，正式实体名称内部文字不参与校验。该检查不代替自然语言时间理解，不自动纠正原文，也不保证覆盖所有省略或模糊日期表达。

### 机构查询权限与数字农商登录

正式查询权限以可信身份映射后的机构编码为准：`PROVINCE_QUERY_ORG_CODES=["省联社正式机构编码"]` 中的机构可查询全部已启用机构，其他机构只能查询自身。默认空列表；管理员角色用于管理操作，不绕过数据机构范围。旧 `CROSS_ORG_QUERY_USER_IDS` 账号白名单已移除，不再生效。

数字农商接入使用已有 `SSO_ENABLED=true` 和 `SSO_USER_INFO_URL`。后端验证平台凭证后自动建立/更新审计所需的影子身份，平台用户不需要维护问数密码。外部机构编码与目录不同时配置 `SSO_ORG_CODE_MAPPING={"平台机构码":"目录机构码"}`；映射后的机构须存在且启用，不按机构名称猜测，不将未知机构默认映射到省联社。

密码登录由独立配置 `PASSWORD_LOGIN_ENABLED` 控制：未配置时保持原行为（启用 SSO 则禁用密码登录）；UAT 设置 `SSO_ENABLED=true`、`PASSWORD_LOGIN_ENABLED=true` 可同时使用数字农商入口和 `/login` 测试账号入口，纯 SSO 投产设置 `PASSWORD_LOGIN_ENABLED=false`。开启密码入口不会为影子账号生成可用密码，测试账号须单独维护；机构权限规则保持一致。用户从数字农商入口携带 token 跳转后进入问数，地址栏立即清理凭证。有效会话刷新使用同源 HttpOnly Cookie 恢复，不重复校验平台 token，不延长原有效期。会话记录登录来源，刷新恢复不改变来源。SSO 会话过期、退出或凭证失效时显示数字农商重新进入提示；密码会话返回密码登录页。前端仅在 sessionStorage 保存入口偏好，不保存令牌且不参与权限判断；旧令牌没有来源字段时按部署的 SSO 模式兼容。可配置 `SSO_PORTAL_URL`（完整 HTTP(S) 平台入口）提供返回按钮；未配置时提示返回平台再次点击入口。

生产环境使用 HTTPS、同源 `/api`，代理必须透传 Cookie 和 Set-Cookie，不缓存认证接口；不能将生产 Secure Cookie 降级为明文 HTTP。当前适用平台跳转顶层页面；若现场以跨站 iframe 嵌入，需另行验证浏览器 Cookie 策略，不能关闭来源校验来绕过。

SIT/UAT 上线核对：用省联社与各行实际平台账号验证映射及查询范围；验证首次跳转、刷新/新标签恢复、退出、过期与重新进入；分别验证密码开关关闭时入口隐藏且服务端拒绝登录、开启时测试账号可登录及刷新恢复。平台原有账号维护保持在数字农商侧，本地影子记录仅用于会话、历史归属和审计。机构变动在下次平台认证时更新影子身份；本系统不承诺在平台停用后即时获知撤销，需平台撤销通知或再次认证，本地会话仍受有效期和会话版本限制。
