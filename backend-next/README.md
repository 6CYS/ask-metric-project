...
仅更换指标时继承当前查询已确认的日期、机构和操作，即使上一轮返回零行数据。
完整新问题仍独立理解；指标有歧义时只澄清指标，不要求重复已确认条件。
多轮模型输入复用单轮指标实体占位保护，返回后恢复原文并校验目录。
新增操作或比较参数必须引用实体名称以外的用户指令；条件来源与分类冲突时纠错重试，
持续冲突则停止执行，不默默删除操作或猜测日期。

### 基础问答配置与升级

基础问答使用以下无敏感值配置，修改后重启后端：

```dotenv
ANALYSIS_ENABLED=false
MULTITURN_V2_ENABLED=true
PROMPT_CONFIG_PATH=config/prompts.query-only.json
```

`config/prompts.query-only.json` 为随源码提供的基础问答提示词模板；`config/prompts.json`
保留分析试验提示词。两份配置都包含新版 `multiturn_context_patch` 实体与条件依赖契约。
如部署使用持久化提示词文件，应备份现有配置，将新版相关提示词合并到实际
`PROMPT_CONFIG_PATH` 指向的文件，保留其他已定制内容；仅升级仓库文件不会覆盖持久化配置。
基础问答模式需同步该模板中的 `intent_routing` 和 `multiturn_context_patch`，避免残留分析指令。
恢复试验时需启用 `ANALYSIS_ENABLED=true` 并使用分析试验配置。
旧错误澄清记录不重写，升级后新建对话验证。

## 归因试验开关

`ANALYSIS_ENABLED=false`（默认值）停用归因执行，保留基础查询、多轮条件修改和历史结果操作。
关闭时归因服务入口拒绝执行，进度接口返回空事件；多轮上下文不继承旧归因任务或内部证据任务。
已有历史记录和试验代码保留，开关变化需重启后端。提示词由 `PROMPT_CONFIG_PATH` 独立指定，
开关不会自动改写提示词；停用分析时须同步使用基础问答提示词。
