...
普通用户按所属机构查询：紫金查紫金、江阴查江阴。省联社、江苏省联合银行的普通用户
可查全部启用机构，在后端用 `ALL_ORGANIZATION_ORG_CODES='["省级机构正式编码"]'`
明确配置；名称和别名不直接授予权限，应先对应正式目录编码。当前目录省联社（全省汇总）
编码为 `320000000`，其他环境须先核对目录。角色仍为 `USER`，管理接口权限不变。
所属机构不存在或停用时拒绝查询；空机构条件仍默认本人机构。
旧 `ALL_ORGANIZATION_USER_IDS` 用户白名单已移除，不能继续授予跨机构权限。
升级时移除旧配置、配置省级机构编码，并将全行普通账号归属到对应省级机构；调整账号
所属机构后撤销旧会话并重新登录，重启后端加载配置。此操作不改变数据湖与数据库结构。
...
`POST /api/v1/data-availability` 受登录、查询就绪状态及机构权限约束。
按数据湖业务日期 `data_dt`（MySQL标准表为 `stat_date`）去重，不使用加载日期 `etl_date`。
未指定指标时不附加指标目录条件；指定指标或机构时校验正式目录并增加绑定条件。
未指定机构时取授权目录范围，显式越权机构拒绝。日期空值排除，不校验指标值或筛选最新批次，
因此它回答“哪些日期有记录”，不保证每项指标值有效。实际取数仍保留原有有效值规则。

请求包含可选 `metric_codes`、`org_codes`、`start`、`end`、`match`（默认 `any`）、
`page`（默认1）、`page_size`（默认10，上限50）。未指定时间表示不限制日期，不能猜测年份。
`any` 为至少一项有记录；`all` 为所有所选机构指标组合均有记录，需要同时明确两侧且最多100个组合。
响应只含一个日期汇总组，不展开逐项覆盖表。总数、最早最新及当前页由同一条SQL统计。
