...
SSO 模式禁止本地密码登录；用户从数字农商入口携带 token 跳转后进入问数，地址栏立即清理凭证。有效会话刷新使用同源 HttpOnly Cookie 恢复，不重复校验平台 token，不延长原有效期。会话过期、退出或凭证失效时显示数字农商重新进入提示，不进入密码表单。可配置 `SSO_PORTAL_URL`（完整 HTTP(S) 平台入口）提供返回按钮；未配置时提示返回平台再次点击入口。
...
SIT/UAT 上线核对：用省联社与各行实际平台账号验证映射及查询范围；验证首次跳转、刷新/新标签恢复、退出、过期与重新进入；确认密码入口隐藏且服务端拒绝密码登录。平台原有账号维护保持在数字农商侧，本地影子记录仅用于会话、历史归属和审计。机构变动在下次平台认证时更新影子身份；本系统不承诺在平台停用后即时获知撤销，需平台撤销通知或再次认证，本地会话仍受有效期和会话版本限制。
