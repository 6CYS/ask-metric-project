WITH normalized AS (
    ...
    WHERE f.{{fact_metric_code_field}} IN :metric_codes
      ...
      AND f.{{fact_data_date_field}} IN (CAST(:current_date AS DATE), CAST(:base_date AS DATE))
),
facts AS (
...
SELECT metric_code, org_code, metric_value, stat_date, metric_increment,
       CASE WHEN stat_date = CAST(:current_date AS DATE) THEN 'current' ELSE 'base' END AS period
FROM facts
ORDER BY metric_code, org_code, stat_date DESC
LIMIT :limit
