...
6. 正文金额统一使用万元：查询 facts 或计算 results 提供 reply_value、reply_unit 时必须直接引用该后端展示值及单位；原始 value、unit 仅供明细与计算证据使用。其他单位保留工具原值与单位，不自行换算。缺少金额展示字段时不要心算换算或猜数值，应说明暂无法生成金额说明并引导查看原始明细。
