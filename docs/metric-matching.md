...
源目录同步保留事实表 `orig_indcr_no`、配置表生成的基础指标名称，以及事实表 `indcr_nm` 取值口径。三者齐全且“基础名称 + 取值口径”与正式名称一致时，优先按可信源结构关联；`hanlp-trie` 在原问句中定位基础名称和口径，保留原文位置。缺少源结构的旧目录只在已命中一个完整正式名称、后续连续省略项与其他完整正式名称有唯一的最长共用前缀、且相邻项之间只有普通连接词时逐项补全；遇到其他实体即停止沿用此前缀。同名多码或多个等强候选继续要求确认。两条路径都只引用当前启用目录的完整名称和编码，不生成新指标或把“增幅”默认拆成公式。完整名称和别名命中仍优先。

例如目录确有“中间业务净收入金额当日数”和“中间业务净收入金额较上月增幅”时，问句“中间业务净收入金额当日数和较上月增幅”会产生两个目录编码。其后若出现另外两个完整指标名称，两个完整指标仍单独保留。若相同基础名称来自不同源指标，省略项须与前面的完整指标保持同一源编码；无法确定源编码、重复名称或重复口径时要求确认。

“A 完整名称、B 省略口径、C 省略口径、D 独立名称”的形式逐项解析；D 若缺字，仍按完整片段的候选分数与歧义规则判断，不能把 D 的末尾口径归给 A，也不能让 D 的短别名掩盖未说完的正式名称。D 未确定时，服务端拒绝只查询 A/B/C，等待用户确认。

确认继续时，Frame 保留 A/B/C 首轮已确定的目录编码和 D 的候选位置；用户只需选择 D 的候选。Agent 用四个编码重新查询当前启用目录，只有全部仍有效才执行一次完整查询。首次请求若模型遗漏了未确定的片段，解析器会保留全部片段并进入澄清，不允许因部分选择而丢失待确认目标；旧目录已确定的 B/C 省略片段也不会被当作独立名称再次检索。

## 整段前缀省略

与上面的口径省略并列的另一条召回通道：按原文标点与空白（与归一化剔除集一致）把问句切成段，若某段归一化后是目录名称或别名的严格前缀，说明用户省略了名称尾部的取值口径（如“保证金存款利息支出金额”之于“保证金存款利息支出金额当日数”）。该通道在不可变索引上为全部归一化名称/别名维护有序前缀表，按前缀二分命中后按编码去重。守卫条件：

- 归一化后段长至少 4 字，更短的通用词头（如“各项存款”类）不进入本通道；
- 必须整段构成严格前缀，段等于完整名称时仍由精确通道处理，段内子串或尾带“是多少”等问句文字不触发；
- 与精确、结构化、口径省略等通道已产出的片段有跨度重叠的分段直接跳过；
- 唯一编码命中直接 resolved；多个编码命中只给 needs_confirmation 候选组，候选为各编码的正式名称，绝不自动选定。

前缀片段与已有片段重叠的模糊候选组不再重复产生；`text`/`start`/`end` 均保留原文坐标。本通道只引用当前启用目录的名称与别名，不生成新指标。

独立新查询的 Frame 必须包含本轮算法识别的所有指标片段。执行前，Agent 将宿主保存的用户原句传给后端；后端按当前启用目录重新解析，发现缺项或未确定项就拒绝创建任务。历史 Frame 的有意选择和后续修改按其引用语义处理，不把原句重新当成独立任务。模型响应中断时，历史投影显示固定的未完成提示，不把半截计划当作最终答案；已保存的步骤结果仍可查看。

这项能力依赖当前启用目录的源字段和同步结果。目录未登记的口径、源字段缺失、同一基础名称跨来源含义不同等情况不能保证自动判定，应确认或维护目录后重试。合成目录测试只能验证算法与接口约束，真实问句和数据库结果仍需环境验收。
...
2026-09-23 前缀省略通道上线后，合成基准 CASES 扩充至 13 条（故障问句复现、唯一/别名前缀正例、“各项存款”类短前缀反例），组合算法 13/13，旧整句与单独 Trie 对新用例均未命中。真实目录的 mention 抽取回归使用 `verification/mention_regression.py`（语料为验收文档真实问句，只读目录）：基线与对照见 `docs/verification/mention-regression-baseline-2026-09-23.json`、`mention-regression-after-prefix-2026-09-23.json`，72 条问句 diff 为零新增、零消失、零状态变化，结论见 `mention-regression-diff-2026-09-23.md`。
...
- 本次多口径省略改造新增 Goldendb Alembic `0005_metric_source_structure`。先审核并应用增量迁移，再用正式目录同步命令预览并回填源字段，确认目标指标已启用且结构一致后升级后端与 Agent。回退应用版本时保留三个可空列及目录数据即可，不需要删除历史结果；如需删除列，应先确认没有旧版本之外的调用方仍依赖它们。
