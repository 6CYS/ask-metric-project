...
当前系统维护单次可信问数、当前任务澄清和历史结果查看/导出；机构贡献度归因功能已移除，意图路由仍识别归因类问题并明确回复不支持。`SemanticEngine.analyze` 中的 analyze 是“语义解析”。跨任务继承、历史指代和追问执行已经退出；历史兼容字段和数据库迁移保留用于读取旧记录。
...
| 9 | [query_execution.py](../backend-next/src/ask_metric/domain/query_execution.py)：`QueryPlanner.build` | 选择支持的查询形态、场景编号和业务参数 |
| 10 | [sql_builder.py](../backend-next/src/ask_metric/infrastructure/query/sql_builder.py)、[mysql.py](../backend-next/src/ask_metric/infrastructure/query/mysql.py)、[inceptor.py](../backend-next/src/ask_metric/infrastructure/query/inceptor.py) | 由 plan 经 SQLGlot 确定性组装 SQL：场景在 `_SCENARIOS` 方言无关注册一次，方言差异收拢在 `_DialectAdapter` 子类（列映射、清洗去重、期间展开），标识符只来自白名单配置，业务值保持绑定占位，由驱动绑定业务值执行只读查询；登记模板由 [templates.py](../backend-next/src/ask_metric/infrastructure/query/templates.py) 保留为 `QUERY_SQL_ENGINE=templates` 回退 |
...
`metric_attribution` 场景、可加性标注与受控计算渲染均已删除；归因类提问由意图路由识别后
统一回复 `INTENT_NOT_AVAILABLE`，不降级为普通取值。
