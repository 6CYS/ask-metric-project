...
当前系统维护单次可信问数和历史结果查看/导出；机构贡献度归因功能已移除。自然语言理解由独立
Node `agent-service`（Pi Agent）完成，后端只提供结构化取数、计算、目录解析与结果读取；
旧后端语义链路（SemanticEngine、澄清协议、外部渠道接口）已整体删除，仅历史兼容字段和
数据库迁移保留用于读取旧记录。
...
`QueryExecutionApplicationService.execute`。没有意图或槽位模型调用，权限、目录、SQL 生成、
结果证据和事务仍走公共执行链；`all_in_range` 复用趋势取数形态，不计算同比或汇总。
基础查询幂等重放会重新核对权限/目录，返回持久化原值而不重复执行 SQL。

自然语言理解在 agent-service 一侧完成：Pi 调用后端 `business-context` 的确定性解析接口
（`resolve-field`、`metric-mentions`、`resolve-scope`）确认指标、机构和日期，再提交
basic-queries 取数。后端 [catalog_references.py](../backend-next/src/ask_metric/domain/catalog_references.py)
的 `explicit_catalog_references` 用 `_protect_resolved_entities` 保护已匹配名称，供历史结果回读时
核对原文所指的机构和指标。
...
```mermaid
flowchart TD
    A[前端提交问题] --> B[agent-service Pi 会话]
    B --> C[Pi 调用 business-context 确定性解析]
    C --> D[Pi 编排 basic-queries 请求]
    D --> E[认证与初始化门禁]
    E --> F[创建 LOGICAL_DSL 任务]
    F --> G[执行前复核权限与目录]
    G --> H[查询计划确定性组装 SQL 及绑定参数]
    H --> I[只读查询数据库]
    I --> J[整理原始结果并保存]
    K[点击历史记录或下载] --> L[核对归属并读取已存结果]
```
...
| 顺序 | 文件与入口 | 它完成什么 |
| --- | --- | --- |
| 1 | [BackendNextChatPanel.vue](../frontend-vue/src/components/chat/BackendNextChatPanel.vue)：`runQuestion` | 把用户输入提交给 agent-service 会话，展示工具事件与结果 |
| 2 | [routes/tasks.py](../backend-next/src/ask_metric/api/routes/tasks.py)：`execute_basic_query`、`calculate_query_facts`、`get_query_task_result` | 将 HTTP 请求转换为业务命令，附带可信身份和幂等键 |
| 3 | [dependencies.py](../backend-next/src/ask_metric/api/dependencies.py)：`require_actor`、`get_query_task_service`、`get_query_execution_service` | 核对身份，并把模型、目录、权限和数据库实现交给服务 |
...
| 5 | [query_execution_service.py](../backend-next/src/ask_metric/application/query_execution_service.py)：`execute`、`_prepare`、`_build_governed_plan` | 复核权限与目录，登记执行，查询并保存结果 |
| 6 | [query_execution.py](../backend-next/src/ask_metric/domain/query_execution.py)：`QueryPlanner.build` | 选择支持的查询形态、场景编号和业务参数 |
| 7 | [sql_builder.py](../backend-next/src/ask_metric/infrastructure/query/sql_builder.py)、[mysql.py](../backend-next/src/ask_metric/infrastructure/query/mysql.py)、[inceptor.py](../backend-next/src/ask_metric/infrastructure/query/inceptor.py) | 由 plan 经 SQLGlot 确定性组装 SQL：场景在 `_SCENARIOS` 方言无关注册一次，方言差异收拢在 `_DialectAdapter` 子类（列映射、清洗去重、期间展开），标识符只来自白名单配置，业务值保持绑定占位，由驱动绑定业务值执行只读查询；所有业务取数与数据覆盖均使用 builder，无模板回退或在线 SQL 编辑 |

例如“查询某机构某天的存款余额”，最终应得到正式指标编码、合法机构集合和确定日期。Pi 负责理解用户意思并编排工具；目录校验、权限复核和查询规划共同决定能否执行。向量相似度高不等于条件已确认，更不等于有权查询。

查询能力由结构判断，不再扫描原句中的“明细、流水”等词直接拒绝。合法但不支持的操作在规划
阶段返回不支持，业务 SQL 不会执行。`detail`、`drill_down` 仍是用来表达
并拒绝超范围请求的类型，不代表系统提供了交易查询功能。

机构贡献度归因已整体移除：`selection=attribution` 不再是合法入参（422 参数校验错误），
`metric_attribution` 场景、可加性标注与受控计算渲染均已删除。

排名“全省各家”缺省范围的下级展开与支行层级钻取共用机构层级数据（`OrgHierarchyProvider`，
恰好一个启用机构无上级时按真实父子关系，否则回退 v1 汇总规则）。支行层级钻取的数据前提用
`scripts/verify_org_hierarchy.py` 在 SIT 环境核实，核实通过后按
`deploy/package/DATA_LAKE_DEPLOY.md` 的“支行层级扩展”段开启 `SIT_ORG_INCLUDE_BRANCH_LEVEL` 并同步目录。
...
每次 basic-queries 调用都建立独立任务。会话编号只是历史列表的归属，不是继承条件的依据；
多轮追问的条件继承由 agent-service 的 Business Frame 完成，后端不读取其他任务的条件。

历史入口是 `get_task`、`get_task_result` 和 `export_task_result`，按用户归属读取已存内容，
...
阻止继续执行，但仍可查看结果。历史任务 JSON 中的 slots/clarification 等字段是旧语义链路遗留，
仅为兼容读取保留。
...
| 指标匹配不对 | `metric_matching.py` 的名称/别名匹配，`metric_candidates.py` 的候选分数与阈值 |
| 日期解析不符合预期 | `time_expression.py` 的 `parse_time_expression`、当前语义配置的必填项 |
