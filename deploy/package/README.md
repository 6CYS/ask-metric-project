...
指标匹配使用 HanLP 独立 Trie、pypinyin 和 RapidFuzz，不部署外部 HanLP 服务或神经模型。
构建器会将只发布源码的 HanLP 轻量依赖预制为纯 Python wheel，再下载目标架构依赖；内网目标机只离线安装。
使用 `--dependency-wheelhouse` 时必须提供包含这些新增依赖的已验证集合。版本、已测架构和边界见
[指标算法匹配与离线部署](../../docs/metric-matching.md)。
...
归因功能已整体移除（含旧原型与受治理归因通道），升级原试验环境须同步前后端并合并持久提示词；详见 [后端升级说明](../../backend-next/README.md#归因功能退出与升级)。已有数据库迁移链不回退。
...
本次需配套发布后端、Agent 和前端，不新增依赖或数据库迁移。先排空活动回合并备份 Agent 数据目录，
先确保后端字段解析接口可用，再切换 Agent/前端。业务 Frame 与结果引用保存在原有 Pi JSONL 目录，
沿用单数据根单写实例约束。不要清理或改写历史文件。
